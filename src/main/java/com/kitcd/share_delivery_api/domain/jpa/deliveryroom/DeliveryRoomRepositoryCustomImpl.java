                ...
                resultList.stream().map(
                        room -> DeliveryRoomDTO.builder()
                                .leader(DeliveryRoomDTO.Leader.builder()
                                        ...
                                        .mannerScore((double)room[1])
                                        .build())
                                .content((String) room[2])
                                .limitPerson(((BigInteger)room[3]).longValue())
                                .storeLink((String)room[4])
                                .status((DeliveryRoomState)room[5])
                                .createdDateTime(((Timestamp) room[6]).toLocalDateTime())
                                .platformType((PlatformType) room[7])
                                .receivingLocation(LocationDTO.builder()
                                        .description((String) room[8])
                                        .address((String)room[9])
                                        .longitude((Double) room[11])
                                        .latitude((Double) room[10])
                                        .build())
                                .deliveryRoomId(bigIntegerObjToLong(room[12]))
                                .deliveryTip(bigIntegerObjToLong(room[13]))
                                .build()
