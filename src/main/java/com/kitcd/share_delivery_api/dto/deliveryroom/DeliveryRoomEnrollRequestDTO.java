...
import com.kitcd.share_delivery_api.utils.url.UrlParserFindStoreName;
...
public class DeliveryRoomEnrollRequestDTO {
    ...
    public DeliveryRoom toEntity(Account leader, ReceivingLocation location, StoreCategory storeCategory){
        UrlParserFindStoreName urlParserFindStoreName = new UrlParserFindStoreName(shareStoreLink, linkPlatformType);
        DeliveryRoom room = DeliveryRoom.builder()
                ...
                .storeName(urlParserFindStoreName.getStoreName())
                .storeLink(urlParserFindStoreName.getUrl())
                .status(DeliveryRoomState.OPEN)
