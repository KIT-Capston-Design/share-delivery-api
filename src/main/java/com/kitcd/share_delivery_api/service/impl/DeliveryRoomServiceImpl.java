...
public class DeliveryRoomServiceImpl implements DeliveryRoomService {
    ...
    @Override
    public DeliveryRoomState deliveryComplete(Long deliveryRoomId) {

        DeliveryRoom deliveryRoom = findByDeliveryRoomId(deliveryRoomId);
        deliveryRoom.checkLeader(ContextHolder.getAccountId());
        DeliveryRoomState newState = deliveryRoom.deliveryComplete();
        ...
        firebaseCloudMessageService.sendMessageTo(
                groupFcmToken,
                deliveryRoom.getContent() + " 배달이 완료되었습니다.",
                "지금 바로 집결지로 모여주세요",
                data
        );
    ...
    public Long deleteDeliveryRoom(Long deliveryRoomId) {
        ...
        Map<String, Object> data = new HashMap<>();
        data.put("type", FCMDataType.DELETE_DELIVERY);
        data.put("roomId", deliveryRoomId.toString());
        ...
        participantFCMTokens.forEach(token -> firebaseCloudMessageService.sendMessageTo(token, "참여한 모집글이 삭제되었습니다.", deliveryRoom.getContent(), data));
